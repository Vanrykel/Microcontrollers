...
#include "light_sensor.h"
    ...
    #define light_sensor_SetP4PinDriveMode(shift, mode)  \
    do { \
        light_sensor_PC =   (light_sensor_PC & \
                                (uint32)(~(uint32)(light_sensor_DRIVE_MODE_IND_MASK << \
                                (light_sensor_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (light_sensor_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define light_sensor_SetP4PinDriveMode(shift, mode)  \
        do { \
            light_sensor_USBIO_CTRL_REG = (light_sensor_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(light_sensor_DRIVE_MODE_IND_MASK << \
                                    (light_sensor_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (light_sensor_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		light_sensor_SetP4PinDriveMode(light_sensor__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(light_sensor_DR & (uint8)(~light_sensor_MASK));
    drVal = (drVal | ((uint8)(value << light_sensor_SHIFT) & light_sensor_MASK));
    light_sensor_DR = (uint32)drVal;
}
...
{
    return (uint8)((light_sensor_PS & light_sensor_MASK) >> light_sensor_SHIFT);
}
...
{
    return (uint8)((light_sensor_DR & light_sensor_MASK) >> light_sensor_SHIFT);
}
...
void light_sensor_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  light_sensor_INTCFG & (uint32)(~(uint32)position);
    light_sensor_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(light_sensor_INTSTAT & light_sensor_MASK);
	light_sensor_INTSTAT = maskedStatus;
    return maskedStatus >> light_sensor_SHIFT;
}
