...
#include "PinUP.h"
    ...
    #define PinUP_SetP4PinDriveMode(shift, mode)  \
    do { \
        PinUP_PC =   (PinUP_PC & \
                                (uint32)(~(uint32)(PinUP_DRIVE_MODE_IND_MASK << \
                                (PinUP_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (PinUP_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define PinUP_SetP4PinDriveMode(shift, mode)  \
        do { \
            PinUP_USBIO_CTRL_REG = (PinUP_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(PinUP_DRIVE_MODE_IND_MASK << \
                                    (PinUP_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (PinUP_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		PinUP_SetP4PinDriveMode(PinUP__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(PinUP_DR & (uint8)(~PinUP_MASK));
    drVal = (drVal | ((uint8)(value << PinUP_SHIFT) & PinUP_MASK));
    PinUP_DR = (uint32)drVal;
}
...
{
    return (uint8)((PinUP_PS & PinUP_MASK) >> PinUP_SHIFT);
}
...
{
    return (uint8)((PinUP_DR & PinUP_MASK) >> PinUP_SHIFT);
}
...
void PinUP_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  PinUP_INTCFG & (uint32)(~(uint32)position);
    PinUP_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(PinUP_INTSTAT & PinUP_MASK);
	PinUP_INTSTAT = maskedStatus;
    return maskedStatus >> PinUP_SHIFT;
}
